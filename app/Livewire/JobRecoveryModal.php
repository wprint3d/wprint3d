...
class JobRecoveryModal extends Component
{
    ...
    public function renderGcode(int $mainMaxLine, int $altMaxLine) {
        if (!$this->printer) {
            ...
            $this->dispatch('recoveryJobPrepareError', message: 'No printer selected.');
        ...
        if (!$this->printer) {
            $this->dispatch('recoveryJobPrepareError', message: 'The printer related to this job was unexpectedly deleted.');
        ...
        if (!$this->printer->activeFile) {
            $this->dispatch('recoveryJobPrepareError', message: 'The printer is in an unexpected state. Did you modify the database manually?');
    ...
    public function recover() {
        ...
        if (
            ...
            $this->dispatch('recoveryFailed', message: 'we\'re still negotiating a connection with this printer, please try again in a few seconds.');
        ...
        if (!$this->printer->connected) {
            $this->dispatch('recoveryFailed', message: 'this printer is not connected.' );
        ...
        try {
            if (!$this->printer->node) {
                $this->dispatch('recoveryFailed', message: 'we don\'t know about this printer\'s node, please try unplugging the USB cable and plugging it back in.');
            ...
            $this->dispatch('recoveryFailed', message: $exception->getMessage() . '.');
        ...
        while ($this->printer->activeFile) {
            ...
            if (time() - $restoreStartTime > $jobRestorationTimeoutSecs) {
                $this->dispatch('recoveryTimedOut');
            ...
            if ($lineNumberCount < $this->printer->lastLine) {
                ...
                foreach (array_keys($absolutePosition) as $key) {
                    if ($absolutePosition[$key] === null) {
                        ...
                        $this->dispatch('recoveryJobPrepareError', message: 'Failed to assert absolute position (not enough context in G-code).');
