...
apt_lock_output="$(
    ...
    LOCK_STATE_FILE="$TEMP_BIN_DIR/lock-state"
...
EOF
    ...
    cat > "$TEMP_BIN_DIR/fuser" <<'EOF'
#!/bin/bash

state=0

if [[ -f "$LOCK_STATE_FILE" ]]; then
    state="$(/bin/cat "$LOCK_STATE_FILE")"
fi

if [[ "$state" -lt 2 ]]; then
    printf '%s' "$((state + 1))" > "$LOCK_STATE_FILE"
    exit 0
fi
...
assert_contains "$apt_lock_output" "Waiting for apt/dpkg locks to clear before continuing Podman setup..."
assert_contains "$apt_lock_output" "provider=podman-compose"
assert_contains "$apt_lock_output" "sleep 3"
assert_contains "$apt_lock_output" "apt-get install -y podman-compose"

echo "container-runtime apt-lock checks passed"
