...
class PluginDependencyServiceTest extends TestCase
{
    ...
    public function test_it_starts_a_candidate_without_interrupting_the_healthy_container_then_promotes_it(): void
    {
        ...
        $this->assertSame('http://acme-bridge-metrics-candidate:9310', $candidate['runtime']['baseUrl']);
        ...
        $this->assertTrue(collect($commands)->contains(function (array $command) use ($candidateService): bool {
            return $command[0] === 'docker'
                ...
                && in_array('acme-bridge-metrics-candidate', $command, true)
        ...
        $this->assertContains(
            ['docker', 'network', 'connect', '--alias', 'acme-bridge-metrics', 'wprint3d_default', $candidateService['candidateContainerName']],
            $commands
        );
        $this->assertContains(
