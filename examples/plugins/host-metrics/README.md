...
```bash
mkdir -p keys
openssl genpkey -algorithm RSA -out keys/host-metrics-private.pem -pkeyopt rsa_keygen_bits:4096
./plugin.sh pack examples/plugins/host-metrics --signing-key=keys/host-metrics-private.pem
```
